...
def balanced(m):
    ...
    if is_weight(m):
        return True
        # check whether left side of the mobile and the right side are balanced
    left_side, right_side = sides(m)
    if is_weight(end(left_side)) and is_weight(end(right_side)):
        return size(end(left_side))*length(left_side) == size(end(right_side))*length(right_side)

    return balanced(end(left_side)) and balanced(end(right_side)) and total_weight(end(left_side))*length(left_side) == total_weight(end(right_side))*length(right_side)
...
def make_fib():
    ...
    current = 0
    def fib():
        # if u use the non local variable, u dont need to put them 
        # in the arguments
        nonlocal count, pre, current
        count += 1
        if count == 1:
            return 0
        elif count == 2:
            pre, current = current, pre+1
        else:
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_pass):
        # nonlocal the balance and create a list to store the incorrect pasword
        nonlocal balance, list_pass, password

        if len(list_pass) == 3:
            return 'Your account is locked. Attempts: {}'.format(list_pass)
        else:
            # check whether the input password is correct
            if input_pass not in password:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
                list_pass.append(input_pass)
...
def make_joint(withdraw, old_password, new_password):
    ...
    # if type(value) != str:
    #     return make_withdraw(value, old_password + new_password)
