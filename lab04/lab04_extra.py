...
def merge(lst1, lst2):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sorted(flatten(lst1)+flatten(lst2))
...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    # notice that filling is from the bottom to the top
    if board[0][column] != '-':
        return (-1,board)
    else:
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    # why do we need the max_rows and max_col
    count = 0
    for j in board[row]:
        if j == player:
            count += 1
        else:
            # careful, here should be 0, not -= 1
            count = 0
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    vertical_win = check_win_column(board, max_rows, max_cols, num_connect, row, player)
